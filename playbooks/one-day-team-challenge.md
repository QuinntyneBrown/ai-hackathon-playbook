...
- Product direction — what the product does, resolved through the PRD (Phase 2)
- Work assignment — who builds what, resolved through capability assignment (Step 10)
...
| Phase | Steps | Output |
| --- | --- | --- |
| 1. Discovery and ideation | 1–2 | Consolidated idea set |
| 2. Definition | 3–7 | Low-fidelity PRD, selected starter template, developed specs, PRD chunked into core capabilities, and cross-capability contracts defined |
| 3. Repository and team setup | 8–13 | Central repository with committed contract skeleton, CI pipeline, protected main, and per-team feature branches |
| 4. Build and integration | 14–15 | Working product on main |

## Phase 1 — Discovery and ideation

### Step 1. Brainstorm in pairs
...
### Step 4. Select a starter template

Select a starter template that matches the product shape the PRD describes — for example, a Clean Architecture solution or a Next.js starter. Selecting the template after the PRD lets the choice follow the product the PRD defines rather than anticipate it. A template removes project setup from the critical path and gives AI coding tools an established structure to extend rather than a blank repository to invent.

### Step 5. Develop the specs

Develop the specs from the low-fidelity PRD. Elaborate each part of the PRD into a specification precise enough to chunk into capabilities and to derive the cross-capability contracts from — the shared domain entities and their persisted shape, the navigation map across the product, and the behavior each part shall deliver.

The facilitator owns the specs and their evolution during the day: the specs are the single reference every team builds against, and when a spec changes the facilitator announces the change to the affected teams. Developing the specs before the PRD is chunked gives every capability a stable description to build against rather than one still forming as the build starts.

### Step 6. Chunk the PRD into core capabilities
...
### Step 7. Define the cross-capability contracts
...
| --- | --- | --- | --- |
| Data model contract | The shared domain entities and their persisted shape | Facilitator | Every capability that reads or writes shared data |
...
| Navigation contract | The route map and the links between pages | Facilitator | Every page |
...
| Spec contract | The PRD chunk boundaries and their ownership | Facilitator | Every capability |
...
A cross-capability contract changes rarely and deliberately: every consuming team rebuilds against it when it changes, so the facilitator arbitrates changes and announces them across teams. Contracts internal to a single capability are not defined here; the team that owns the capability defines them under the [Parallel agent-assisted build](parallel-agent-build.md) workflow.

## Phase 3 — Repository and team setup

### Step 8. Set up a central repository

Create a central git repository from the starter template and push it to the shared host.

### Step 9. Commit the cross-capability contract skeleton
...
### Step 13. Protect main and set up the CI pipeline
...
### Step 14. Review and merge pull requests
...
### Step 15. Iterate until complete
...
- Developing the specs before the PRD is chunked gives every team a single, stable description of the product to build against, and keeps ownership of that description with the facilitator rather than dividing it across the parallel teams.
